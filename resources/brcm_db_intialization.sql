...
CREATE SEQUENCE public.order_receipt_order_id_seq;

CREATE TABLE public.order_receipt (
                order_id INTEGER NOT NULL DEFAULT nextval('public.order_receipt_order_id_seq'),
                time TIME NOT NULL,
                date DATE NOT NULL,
...
ALTER SEQUENCE public.order_receipt_order_id_seq OWNED BY public.order_receipt.order_id;

CREATE TABLE public.order_item (
                order_id INTEGER NOT NULL,
                name VARCHAR(40) NOT NULL,
                quantity INTEGER NOT NULL,
                CONSTRAINT order_id_item_id PRIMARY KEY (order_id, name)
);
COMMENT ON COLUMN public.order_item.quantity IS 'Number of hours the customers intends to be performing activity';


ALTER TABLE public.order_item ADD CONSTRAINT item_order_item_fk
FOREIGN KEY (name)
...
ALTER TABLE public.order_receipt ADD CONSTRAINT status_order_fk
FOREIGN KEY (label)
...
ALTER TABLE public.order_receipt ADD CONSTRAINT customer_order_fk
...
ALTER TABLE public.order_item ADD CONSTRAINT order_order_item_fk
FOREIGN KEY (order_id)
REFERENCES public.order_receipt (order_id)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;
